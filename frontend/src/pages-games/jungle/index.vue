  ...
  <view class="jungle">
    ...
    <view v-else class="jungle__room">
      ...
      <view class="jungle__actions">
        ...
        <view class="jungle__action" hover-class="press" @tap="onBack">
          <text class="jungle__action-icon">🚪</text>
        </view>
      </view>
...
watch(
  ...
  (key, prevKey) => {
    // 字符串 key 值比较：数组 getter 每次返回新引用，会让每个动作（走子/聊天/WS 推送）都重触发
    // 回调，曾导致对局中每操作一次就重弹一次猜拳定格卡——必须按值比较。
...
const ANIM_MS = 250
const anim = ref<{ side: JungleSide; animal: JungleAnimal; fr: number; fc: number; tr: number; tc: number; startMs: number } | null>(null)
let animTimer: ReturnType<typeof setInterval> | null = null

function startMoveAnim(next: JungleRoomState, lm: JungleLastMove): void {
  const moved = pieceAt(next.pieces, lm.tr, lm.tc)
  if (!moved) {
    drawBoard()
    return
  }
  anim.value = { side: moved.side, animal: moved.animal, fr: lm.fr, fc: lm.fc, tr: lm.tr, tc: lm.tc, startMs: Date.now() }
...
function drawBoard() {
  ...
  // 棋子（水中鼠加白圈）——落子动画期间跳过终点上的移动子，改为插值位置绘制
  for (const piece of current.pieces) {
    if (anim.value && piece.r === anim.value.tr && piece.c === anim.value.tc && piece.side === anim.value.side && piece.animal === anim.value.animal) continue
    drawPiece(ctx, piece)
  }
  ...
  if (lm) {
    const startCell = cellRect(lm.fr, lm.fc, metrics)
    ctx.strokeStyle = COLOR_GOLD
    ctx.globalAlpha = anim.value ? 0.85 : 0.45
    ctx.lineWidth = 2 * s
    ctx.setLineDash([4 * s, 3 * s])
    ctx.beginPath()
    ctx.arc(startCell.x + startCell.size / 2, startCell.y + startCell.size / 2, 15 * s, 0, Math.PI * 2)
    ctx.stroke()
    ctx.setLineDash([])
    ctx.globalAlpha = 1
  }
  if (anim.value) {
    ...
    const from = cellRect(a.fr, a.fc, metrics)
    const to = cellRect(a.tr, a.tc, metrics)
    drawPieceCenter(
      ctx,
      from.x + (to.x - from.x) * eased + to.size / 2,
      from.y + (to.y - from.y) * eased + to.size / 2,
      a.side,
      a.animal,
      isRiver(a.tr, a.tc),
    )
  } else if (lm) {
    const targetCell = cellRect(lm.tr, lm.tc, metrics)
    ctx.strokeStyle = COLOR_GOLD
    ctx.lineWidth = 2.5 * s
    ctx.beginPath()
    ctx.arc(targetCell.x + targetCell.size / 2, targetCell.y + targetCell.size / 2, 23 * s, 0, Math.PI * 2)
    ctx.stroke()
  }
}
...
function drawPiece(ctx: CanvasRenderingContext2D, piece: { side: JungleSide; animal: JungleAnimal; r: number; c: number }) {
  ...
  drawPieceCenter(ctx, x + size / 2, y + size / 2, piece.side, piece.animal, isRiver(piece.r, piece.c))
}

/** 以圆心坐标绘制棋子（落子动画的插值位置也走这里）。 */
function drawPieceCenter(ctx: CanvasRenderingContext2D, cx: number, cy: number, side: JungleSide, animal: JungleAnimal, onWater: boolean) {
  const s = cell / 45
  const red = side === 'red'
  ...
  ctx.textAlign = 'center'
  ctx.textBaseline = 'middle'
  ctx.fillText(ANIMAL_CN[animal], cx, cy - 1 * s)
  ...
  ctx.fillText(String(RANKS[animal]), bx, by + 0.5 * s)
}
...
watch(
  ...
  (next) => {
    ...
    // 有新的一手 → 播放滑动动画（双方走子都播，对方也能一眼看到动了哪个子）；其余变化直接重绘
    const lm = next.lastMove
    if (!firstLoad && next.ply > prevPly && lm) startMoveAnim(next, lm)
    else drawBoard()
...
onShareAppMessage(() => ({
  ...
  path: state.value ? `/pages-games/jungle/index?room=${state.value.code}` : '/pages-games/jungle/index',
}))
</script>
