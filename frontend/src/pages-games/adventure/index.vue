      ...
      <view class="chat-panel" @tap.stop>        <view class="chat-panel-tabs">
        ...
        <scroll-view class="chat-log" scroll-y :show-scrollbar="false">
          <view v-for="m in chatLog" :key="m.seq" class="chat-log-row" :class="{ 'chat-log-me': m.seat === mySeat }">
            ...
            <text class="chat-log-text" :class="{ 'chat-log-emoji': m.kind === 'emoji' }">{{ m.kind === 'sticker' ? '[贴纸]' : chatBody(m) }}</text>
          </view>
        </scroll-view>
...
const recentChats = computed(() => (current.value?.chat ?? []).slice(-6))
...
onShareAppMessage(() => ({
  title: '枫趣冒险 · 一起来登枫叶山',
  path: current.value?.sharePath ?? '/pages-games/adventure/index',
}))
</script>
