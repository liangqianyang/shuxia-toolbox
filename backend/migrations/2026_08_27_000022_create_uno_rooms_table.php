...
return new class extends Migration {
    ...
    {
        Schema::create('uno_rooms', function (Blueprint $table): void {
            ...
            $table->json('state')->default(new Expression('(JSON_OBJECT())'))->comment('完整对局快照（deck/hands/discard/direction/currentSeat 等，服务端权威）');
            $table->unsignedBigInteger('version')->default(0)->comment('状态版本号，每次变更 +1');
            $table->unsignedBigInteger('winner_user_id')->nullable()->comment('胜者用户 id');
            $table->string('win_reason', 16)->nullable()->comment('cards/forfeit/last_man');
            $table->datetime('turn_deadline_at')->nullable()->comment('当前回合截止时间（20s 回合计时）');
        ...
        Db::table('tool_catalog')->updateOrInsert(
            ...
            [
                ...
                'icon' => '/static/icons/uno-1.png',
                'route' => '/pages/uno/index',
                'category' => 'game',
                'is_published' => true,
                'sort_order' => 70,
    ...
    {
        Schema::dropIfExists('uno_rooms');
        Db::table('user_tool_preferences')->where('tool_key', 'uno')->delete();
        Db::table('tool_catalog')->where('tool_key', 'uno')->delete();
    }
};
