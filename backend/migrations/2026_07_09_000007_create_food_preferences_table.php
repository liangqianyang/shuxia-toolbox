...
return new class extends Migration {
    ...
    {
        Schema::create('food_preferences', function (Blueprint $table): void {
            $table->bigIncrements('id')->comment('主键');
            $table->unsignedBigInteger('user_id')->default(0)->comment('所属用户 id');
            $table->string('client_id', 80)->default('')->comment('前端生成的标签 id');
            $table->string('label', 40)->default('')->comment('标签名称');
