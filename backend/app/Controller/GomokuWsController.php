...
use App\Service\Gomoku\GomokuWsPusher;
...
/**
 * 五子棋 WebSocket 通道：连接管理 + 状态推送。
 *
 * 鉴权放在 query（wx.connectSocket 的 header 在部分平台不生效）：
 * /gomoku/ws?apiKey=…&token=…&code=1234
 * 对局变更一律走 HTTP 接口（服务端权威），这里只收心跳、推状态。
 */
final class GomokuWsController implements OnMessageInterface, OnOpenInterface, OnCloseInterface
{
    ...
    public function onOpen($server, $request): void
    {
        ...
        $this->logger->info(sprintf('[gomoku] ws open: room=%s user=%d fd=%d', $code, $userId, $fd));
    }
    ...
    public function onClose($server, int $fd, int $reactorId): void
    {
        ...
        $this->logger->info(sprintf('[gomoku] ws close: fd=%d', $fd));
    }
}
