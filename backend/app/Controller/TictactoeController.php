...
/** 联机井字棋：房间创建/加入、首局猜拳定 X、轮询同步、落子、再来一局（换先手）与离开。 */
final class TictactoeController extends AbstractController
{
    ...
    public function move(string $code, RequestInterface $request): array
    {
        ...
        return $this->ok($this->rooms->move($code, $this->requireUserId($request), $index));
    }
