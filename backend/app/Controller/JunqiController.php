...
/** 联机军棋（两人暗棋）：房间创建/加入、布阵、猜拳定先手、轮询同步、走子、再来一局与离开。 */
final class JunqiController extends AbstractController
{
    ...
    public function layout(string $code, RequestInterface $request): array
    {
        $layout = $request->input('pieces');
        if (! is_array($layout)) {
            throw new BizException(422, '阵型数据不正确');
        }
        return $this->ok($this->rooms->layout($code, $this->requireUserId($request), $layout));
    }
