...
use App\Service\Ludo\LudoWsPusher;
...
/**
 * 飞行棋 WebSocket 通道：连接管理 + 状态推送。
 *
 * 鉴权放在 query（wx.connectSocket 的 header 在部分平台不生效）：
 * /ludo/ws?apiKey=…&token=…&code=1234
 * 对局变更一律走 HTTP 接口（服务端权威），这里只收心跳、推状态。
 * 契约接口参数必须保持无类型（见 uno 同名类的说明），加 Swoole 类型声明会 fatal。
 */
final class LudoWsController implements OnMessageInterface, OnOpenInterface, OnCloseInterface
{
    ...
    public function onOpen($server, $request): void
    {
        ...
        $this->logger->info(sprintf('[ludo] ws open: room=%s user=%d fd=%d', $code, $userId, $fd));
    }
    ...
    public function onClose($server, int $fd, int $reactorId): void
    {
        ...
        $this->logger->info(sprintf('[ludo] ws close: fd=%d', $fd));
    }
}
