...
/**
 * UNO 房间 WebSocket 连接表 + 状态推送。
 *
 * worker_num=1，进程内数组即可保证全房间连接可见；
 * 只推送，不做权威判定——对局变更走 UnoRoomService（HTTP），这里广播结果。
 * 手牌等隐藏信息由 UnoRoomService::serialize 按用户视角裁剪，本类只负责按 fd 逐个序列化推送。
 */
final class UnoWsPusher
{
    ...
    /**
     * 向房间内所有连接推送完整状态（每个连接看到的手牌不同，按 fd 对应用户逐个序列化）。
     *
    ...
    public function pushRoom(string $code, callable $stateFactory): void
    {
        ...
        foreach ($fds as $fd => $userId) {
            ...
            try {
                ...
                $this->leave($fd);
                $this->logger()?->warning('[uno] ws push failed for fd ' . $fd . ': ' . $e->getMessage());
            }
        }
    }
