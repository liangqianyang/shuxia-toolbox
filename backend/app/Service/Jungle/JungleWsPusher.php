...
/**
 * 斗兽棋房间 WebSocket 连接表 + 状态推送。
 *
 * worker_num=1，进程内数组即可保证全房间连接可见；
 * 只推送，不做权威判定——对局变更走 JungleRoomService（HTTP），这里广播结果。
 */
final class JungleWsPusher
{
    ...
    public function pushRoom(string $code, callable $stateFactory): void
    {
        ...
        foreach ($fds as $fd => $userId) {
            ...
            try {
                ...
                $this->leave($fd);
                $this->logger()?->warning('[jungle] ws push failed for fd ' . $fd . ': ' . $e->getMessage());
            }
        }
    }
