...
/**
 * 联机斗兽棋房间：服务端权威，状态存 MySQL（重启不丢局）。
 *
 * 所有写操作走事务 + 行锁，杜绝双击/竞态分叉棋局；
 * 每次写操作提交后经 JungleWsPusher 向房间内 WebSocket 连接广播最新状态。
 * 红蓝列在猜拳定选边前只是临时标签（创建者先坐红），最终执子由胜者选边决定，红方先行。
 * 对局走子不限时（照五子棋）；仅猜拳/选边窗口有超时（Timer 清扫 + 写路径懒推进兜底）。
 */
final class JungleRoomService
{
    /** 房间闲置多久（秒）后懒清理。 */
    ...
    /** 聊天冷却（秒）/ 环形保留条数（同 uno/冒险棋/飞行棋/五子棋）。 */
    ...
    private const array MOVE_ERRORS = [
        ...
        'own_den' => '不能走进自己的兽穴',
        'into_water' => '只有鼠能下河',
        'blocked_own' => '落点是自己的动物',
        'not_adjacent' => '一次只能走一格',
        'jump_invalid' => '狮虎只能横着跳过河',
        'jump_blocked' => '河里有鼠挡道，跳不过去',
        'cannot_capture' => '吃不掉对方的动物',
    ];

    public function __construct(
        private readonly WechatUserService $users,
        private readonly JungleWsPusher $pusher,
        private readonly FeatureFlagService $flags,
        private readonly WechatContentSecurityService $security,
    ) {}

    /**
     * 创建房间：创建者先临时坐红（红蓝只是猜拳前的标签），最终执子由开局猜拳定选边。
     * 插入前顺手清理 24h 未更新的旧房。
     *
    ...
    public function create(int $userId): array
    {
        ...
        $room = Db::transaction(function () use ($userId) {
            ...
            $room->blue_user_id = 0;
            $room->status = 'waiting';
            $room->pieces = JungleRule::initialPieces();
            $room->turn = null;
            $room->last_move = null;
            $room->version = 1;
            $room->winner = null;
    ...
    public function join(string $code, int $userId): array
    {
        $room = Db::transaction(function () use ($code, $userId) {
            ...
            if ($room->status === 'waiting' && ($room->red_user_id === 0 || $room->blue_user_id === 0)) {
                ...
                // 双人坐满 → 猜拳定选边（胜者选执红/执蓝），选完才正式开局（红先）
                $room->status = 'rps';
    ...
    /**
     * 胜者选边（rps 阶段）：选执红先手或执蓝后手；超时未选默认执红。
     *
     * @return array<string, mixed>
     */
     ...
     * （出拳期到 → 代未出者随机出并结算；选边期到 → 默认执红开局）。返回推进的房间数。
     */
    public function sweepDueRpsRooms(): int
    {
        $codes = JungleRoom::query()
            ->where('status', 'rps')
    ...
    /**
     * 走子：校验回合与规则（跳河/水陆隔离/陷阱/鼠象特例），吃子即时结算；
     * 入对方兽穴或吃光对方即胜，对方困毙（无合法着法）亦胜。返回最新完整状态。
     *
     * @return array<string, mixed>
     */
    public function move(string $code, int $userId, int $fr, int $fc, int $tr, int $tc): array
    {
        $room = Db::transaction(function () use ($code, $userId, $fr, $fc, $tr, $tc) {
            ...
            $pieces = $room->pieces ?? [];
            $error = JungleRule::validateMove($pieces, $role, $fr, $fc, $tr, $tc);
            if ($error !== null) {
                throw new BizException(422, self::MOVE_ERRORS[$error] ?? '这步走不了');
            }

            [$nextPieces, $captured] = JungleRule::applyMove($pieces, $fr, $fc, $tr, $tc);
            ...
            $winReason = JungleRule::findWin($nextPieces, $role, $tr, $tc);
            if ($winReason === null && ! JungleRule::hasAnyMove($nextPieces, $this->opponentOf($role))) {
                $winReason = 'stuck';
            }
            if ($winReason !== null) {
    ...
    /**
     * 再来一局：终局后由任一入座玩家发起，重置盘面并重新猜拳定选边
     * （红蓝列只作猜拳前的临时标签，最终执子由胜者选边决定）；聊天记录保留。
     *
     * @return array<string, mixed>
     */
    ...
    public function serialize(JungleRoom $room, int $requesterId): array
    {
        $pieces = $room->pieces ?? [];
     ...
     *
     * @param string $myRole 'red'|'blue'|'spectator'
     * @return null|array<string, mixed>
     */
    private function serializeRps(JungleRoom $room, string $myRole): ?array
    {
        $rps = $room->rps;
        if (! is_array($rps)) {
            return null;
    ...
    private function chatSeqOf(JungleRoom $room): int
    {
        $chat = $room->chat ?? [];
    ...
    private function seatedRole(JungleRoom $room, int $userId): ?string
    {
    ...
    private function requireRpsRole(JungleRoom $room, int $userId): string
    {
        $role = $this->seatedRole($room, $userId);
        if ($role === null) {
            throw new BizException(403, '你不是本局玩家');
     ...
     */
    private function resolveRps(JungleRoom $room): void
    {
        $rps = $room->rps;
    ...
    /**
     * 应用选边结果：胜者要的颜色若与当前临时座位不符则交换红蓝列（连 seen_at 一起），
     * 进入正式对局（红先）；rps 保留 winner/picks/chosen 供前端展示定格。
     */
    private function applyColorChoice(JungleRoom $room, string $color): void
    {
        $rps = $room->rps;
        $winner = (string) $rps['winner'];
        if ($winner !== $color) {
            [$room->red_user_id, $room->blue_user_id] = [$room->blue_user_id, $room->red_user_id];
            [$room->red_seen_at, $room->blue_seen_at] = [$room->blue_seen_at, $room->red_seen_at];
        }
        $rps['chosen'] = $color;
        $room->rps = $rps;
        $room->status = 'playing';
        $room->turn = JungleRule::RED;
        $room->turn_deadline_at = null;
    }
     ...
     */
    private function applyDueRpsIfNeeded(JungleRoom $room, ?int $exceptUserId = null): bool
    {
        if ($room->status !== 'rps' || $room->turn_deadline_at === null) {
            return false;
        }
        ...
        if ($phase === 'pick') {
            ...
            // 选边超时：默认执红（红先）
            $this->applyColorChoice($room, 'red');
        }
        return true;
    }

    /** 更新入座玩家的 seen_at 心跳；不 bump version，避免心跳搅动同步计数。 */
    private function touchSeenAt(JungleRoom $room, int $userId): void
    {
        $now = date('Y-m-d H:i:s');
    ...
    private function lockByCode(string $code): JungleRoom
    {
        $room = JungleRoom::query()->where('code', $this->normalizeCode($code))->lockForUpdate()->first();
        if (! $room instanceof JungleRoom || $room->status === 'closed') {
    ...
    /** 取活跃房间（无锁，读路径）。 */
    private function findActive(string $code): JungleRoom
    {
        $room = JungleRoom::query()->where('code', $this->normalizeCode($code))->first();
        if (! $room instanceof JungleRoom || $room->status === 'closed') {
            throw new BizException(404, '房间不存在或已结束');
        }
        return $room;
    ...
    private function newCode(): string
    {
        for ($i = 0; $i < 16; $i++) {
            ...
            $exists = JungleRoom::query()->where('code', $code)->where('status', '!=', 'closed')->exists();
            if (! $exists) {
                return $code;
            }
        }
