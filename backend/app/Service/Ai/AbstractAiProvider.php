...
/**
 * AI 行程生成的厂商无关部分：prompt 构建 + 响应解析 + 三个公开入口。
 * 厂商差异（请求 URL / 鉴权 / 联网能力）由子类实现 requestAiContent() 与 webSearchEnabled()。
 */
abstract class AbstractAiProvider implements AiProvider
{
    /** 对齐前端 PoiType；AI 返回的非法 type 一律回退 sight */
    protected const array ALLOWED_TYPES = ['sight', 'food', 'stay', 'shop', 'transit'];

    /** 对齐前端 DayMood；AI 返回非法值时回退城市漫游 */
    protected const array ALLOWED_DAY_MOODS = ['citywalk', 'nature', 'culture', 'food', 'family', 'couple', 'classic'];

    /** 出行方式中文标签（写进 prompt，让 AI 按此规划路线与站间衔接） */
    protected const array MODE_LABELS = [
        'walking' => '步行',
        'cycling' => '骑行',
        'driving' => '驾车/打车',
        'transit' => '公共交通（地铁/公交）',
    ...
    protected const array INTENSITY_LABELS = [
        'relaxed' => '轻松慢游',
        'standard' => '标准舒适',
        'packed' => '充实打卡',
    ];
    ...
    protected const array INTENSITY_RULES = [
    ...
    /**
     * 发送 chat/completions 请求并返回 message.content。
     *
     * @param array<int, array{role: string, content: string}> $messages
     */
    abstract protected function requestAiContent(array $messages, bool $webSearch): string;

    /**
     * 当前厂商/配置下是否启用联网搜索。不支持联网的厂商必须返回 false，
     * 让 prompt 自动去掉「联网搜索」表述，避免模型假装联网编造数据。
     */
    abstract protected function webSearchEnabled(): bool;
     ...
     * 兼容模型把数字写成带单位的字符串（如 "120分钟"、"约 280 公里"）：取首段连续数字。
     */
